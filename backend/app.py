...
def generate():
    ...
    " - optimized_resume: a concise, ATS-friendly resume tailored to the job, formatted as valid LaTeX code. "
    "IMPORTANT: Properly escape all LaTeX special characters (\\, &, %, $, #, _, {, }, ~, ^). "
    "The LaTeX must be complete and compilable with \\documentclass{resume}.\n"
    ...
    "Keep each field concise. Return valid JSON parsable by a machine.\n"
    )
...
def convert_latex_to_pdf():
    ...
    try:
        ...
        pdf_bytes = convert_tex_to_pdf_direct(
            temp_tex_path,
            r'C:/hackcamp-project/backend/LaTex/resume.cls'
        )
